...
bool ActionSpace::isOccupied() const {
    return !occupiedPlayerIds.empty();
}

bool ActionSpace::isOccupiedByPlayer(int playerId) const {
    return std::find(occupiedPlayerIds.begin(), occupiedPlayerIds.end(), playerId) != occupiedPlayerIds.end();
}

bool ActionSpace::execute(Player& player) {
    if (isOccupied()) {
        return false;
    }
...
void ActionSpace::reset() {
    occupiedPlayerIds.clear();
}