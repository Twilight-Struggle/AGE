    ...
    , resources()
{
    ...
    // 初期食料の設定
    resources[static_cast<size_t>(ResourceType::FOOD)] = Resource(ResourceType::FOOD, 3);
}
...
bool Player::hasResource(const Resource& resource) const {
    return resources[static_cast<size_t>(resource.getType())].getAmount() >= resource.getAmount();
}
...
int Player::getFamilyMemberCount() const {
...
const Farm& Player::getFarm() const {
    return farm;
}

const int Player::getBegginCards() const {
    return begginCards;
}