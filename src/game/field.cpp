...
void Field::plow() { type = FieldType::FIELD; }

void Field::fence() {
  // 現在の状態に PASTURE フラグを追加
  type = static_cast<FieldType>(static_cast<int>(type) |
...
void Field::buildWoodRoom() { type = FieldType::WOOD_ROOM; }

void Field::buildClayRoom() { type = FieldType::CLAY_ROOM; }

void Field::buildStoneRoom() { type = FieldType::STONE_ROOM; }
...
bool Field::addContent(const Resource& resource) {
  ...
  if ((isLivestock && type != FieldType::PASTURE) ||
      (isCrop && type != FieldType::FIELD) || (!isLivestock && !isCrop)) {
    return false;
  }
